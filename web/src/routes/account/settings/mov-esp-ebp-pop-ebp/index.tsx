...
export default function () {
  ...
  async function handleDeactivate(result: CaptchaForm) {
    ...
    try {
      ...
      handleHttpError(err as Error, t("account.settings.delete.failed")!);
  ...
  return (
    <>
      <Title page={t("account.settings.delete.title")} route="/account/settings/mov-esp-ebp-pop-ebp" />
        ...
        <div class="flex-1 flex flex-col max-w-5xl space-y-2">
          <div class="pt-4 md:p-12 md:pb-4 flex flex-row md:flex-col items-center justify-center">
            ...
            <h1 class="text-center text-lg font-bold text-error ml-4 md:ml-0 md:mt-4">
              {t("account.settings.delete.topWarning")}
            </h1>
          </div>
          ...
          <article class="article w-full max-w-5xl self-center mt-4">
            <p>
              <strong>{t("account.settings.delete.tips1")}</strong>
            </p>
            <ul>
              <li>{t("account.settings.delete.tipsList1")}</li>
              <li>{t("account.settings.delete.tipsList2")}</li>
              <li>{t("account.settings.delete.tipsList3")}</li>
              <li>{t("account.settings.delete.tipsList4")}</li>
            </ul>
            <p>
              <strong>{t("account.settings.delete.tips3")}</strong>
            </p>
            <p class="text-error">{t("account.settings.delete.tips2", { name: accountStore.account! })}</p>
          </article>
          <Divider class="w-full" />
          <Form
            ...
            <Input
              ...
              extraBtn={
                <Button class="rounded-l-none text-error" disabled={!canDelete() || loading()} loading={loading()}>
                  ...
                  <span class="hidden md:inline">{t("account.settings.delete.bye")}</span>
                </Button>
              }
              class="flex-1 ml-2"
              onInput={(v) => {
