...
export default function () {
  ...
  const currentMonthDays = createMemo(() => {
    ...
    const days = []
    const prevMonth = currentDate.minus({ month: 1 })
    const currentMonth = currentDate
    const currentMonthDays = currentMonth.daysInMonth
    const currentMonthFirstDay = currentMonth.startOf('month').weekday
    const prevMonthDays = prevMonth.daysInMonth
    const prevMonthTail = prevMonthDays - currentMonthFirstDay
    const nextMonthHead = 7 - ((currentMonthFirstDay + currentMonthDays) % 7)
    ...
    <>
        ...
        <Card contentClass="p-2 flex flex-col space-y-2">
          ...
          <div class="grid grid-cols-7 gap-2">
            ...
            {Array.from({ length: 7 }).map((_, index) => (
              <div class="h-12 w-12 flex justify-center items-center font-bold opacity-60">
      ...
      <div class="flex flex-col flex-1 items-center">
        <div class="flex flex-col w-full max-w-5xl flex-1">
          <h1 class="font-bold text-center text-xl hidden lg:inline-block">{t('calendar.games')}</h1>
          <Show when={events().length === 0}>
            <div class="flex-1 flex items-center justify-center space-x-2 opacity-60">
              <span class="icon-[fluent--person-walking-20-regular] w-5 h-5"></span>
              <span class="text-lg">{t('calendar.noGames')}</span>
            </div>
          </Show>
        </div>
      </div>
